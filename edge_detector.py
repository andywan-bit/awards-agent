...
import requests
from config import KALSHI_API_KEY, EDGE_ALERT_THRESHOLD, MIN_CONFIDENCE
...
def fetch_kalshi_price(nominee: str) -> int | None:
    """Fetch live Kalshi price for a nominee. Falls back to demo data."""
    if KALSHI_API_KEY is None:
        return DEMO_KALSHI.get(nominee)

    try:
        resp = requests.get(f"{BASE_URL}/markets", headers={
            "Authorization": f"Bearer {KALSHI_API_KEY}",
        }, params={"status": "open", "limit": 200}, timeout=10)
        resp.raise_for_status()
        markets = resp.json().get("markets", [])
        key = nominee.lower().split("–")[0].strip().split()[-1]
        for m in markets:
            if key in m.get("title", "").lower():
                return m.get("yes_ask", 50)
...
def find_edges(precursor_data: dict) -> list[dict]:
    ...
    for nominee, signals in precursor_data.items():
        ...
        kalshi_price = fetch_kalshi_price(nominee)
